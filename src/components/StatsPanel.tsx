...
import { formatNumber, formatEfficiency, formatPercentage, getEfficiencyChange } from '../lib/formatting';
...
export function StatsPanel({ stats }: StatsPanelProps) {
  const efficiencyChange = getEfficiencyChange(stats.powerPerGold, stats.previousPowerPerGold);
  const EfficiencyArrow = efficiencyChange.direction === 'increase' ? ArrowUpRight : ArrowDownRight;
  const efficiencyColor = efficiencyChange.direction === 'increase' ? 'text-green-400' : 'text-red-400';
      ...
      <div className="bg-galaxy-900/50 backdrop-blur-sm rounded-xl p-6 border border-galaxy-700/50 shadow-neon">
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="text-2xl font-bold text-galaxy-100">{formatNumber(stats.currentPower)}</div>
          </div>
      ...
      <div className="bg-galaxy-900/50 backdrop-blur-sm rounded-xl p-6 border border-galaxy-700/50 shadow-neon">
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="text-2xl font-bold text-galaxy-100">{formatNumber(stats.totalGoldSpent)}</div>
          </div>
          {stats.nextLevelCost && (
            <div>
