...
import { Map, TrendingUp, AlertTriangle, CheckCircle, XCircle, Clock, Coins, ChevronDown, ChevronUp, Sparkles } from 'lucide-react';
import { useStarStore } from '../../store/starStore';
import { formatNumber } from '../../lib/formatting';

export function UnifiedStarDisplay() {
  const [showDetails, setShowDetails] = useState(false);
  const { selectedConstellation, starAnalysis } = useStarStore();

  if (!selectedConstellation || !starAnalysis) {
    return (
  ...
  const StatusIcon = starAnalysis.status === 'success' ? CheckCircle : 
                    starAnalysis.status === 'warning' ? AlertTriangle : XCircle;
  ...
  return (
    <div className={`bg-galaxy-900/50 backdrop-blur-sm rounded-xl p-6 border shadow-neon
      ${starAnalysis.status === 'success' ? 'border-green-500/50' :
        starAnalysis.status === 'warning' ? 'border-yellow-500/50' : 'border-red-500/50'}`}>
      <div className="flex items-center gap-3 mb-4">
        <StatusIcon className={`w-6 h-6
          ${starAnalysis.status === 'success' ? 'text-green-400' :
            starAnalysis.status === 'warning' ? 'text-yellow-400' : 'text-red-400'}`} />
        <h3 className="font-semibold text-galaxy-100">{starAnalysis.message}</h3>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="flex mb-2 items-center justify-between">
            ...
            <div className="text-right">
              <span className="text-sm font-semibold inline-block text-galaxy-100">
                {starAnalysis.returnPercentage.toFixed(1)}%
              </span>
            </div>
          </div>
          <div className="overflow-hidden h-2 text-xs flex rounded bg-galaxy-800">
            <div
              style={{ width: `${Math.min(starAnalysis.returnPercentage, 100)}%` }}
              className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center
                ${starAnalysis.status === 'success' ? 'bg-green-500' :
                  starAnalysis.status === 'warning' ? 'bg-yellow-500' : 'bg-red-500'}`}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="text-xl font-bold text-galaxy-100">
              {formatNumber(starAnalysis.expectedGoldGain)}
            </div>
          </div>
        ...
        <div className="p-3 bg-galaxy-800/50 rounded-lg border border-galaxy-700/30">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-yellow-400" />
            <span className="text-sm text-galaxy-300">Element matching heroes gain +20% power</span>
          </div>
        </div>
              ...
              <div className="relative pt-1">
                <div className="flex mb-2 items-center justify-between">
                  <span className="text-sm font-semibold text-galaxy-100">
                    {starAnalysis.progressToNextConstellation.toFixed(1)}%
                  </span>
                </div>
                <div className="overflow-hidden h-2 text-xs flex rounded bg-galaxy-800">
                  <div
                    style={{ width: `${Math.min(starAnalysis.progressToNextConstellation, 100)}%` }}
